...
data_path = "/lustre/scratch/data/tweissbe_hpc-becos-all/scape_o/test"
...
with torch.no_grad():
    for i in tqdm(os.listdir(data_path)):
        ...
        torch.save(cos_similarity, "/lustre/scratch/data/s95wwang_hpc-chirality_image/matching_scape_corres_prediction/" + mode  + str(i) + ".pt")

    df = pd.DataFrame(tracks)
    df.insert(0, "pair", pairs)
    df.to_csv("shape_matching_results_scape_" + mode + ".csv", index=False)