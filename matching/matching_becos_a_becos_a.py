...
with torch.no_grad():
    for i in tqdm(os.listdir(data_path)):
        if i in animal_list:
            ...
            torch.save(cos_similarity, "/lustre/scratch/data/s95wwang_hpc-chirality_image/matching_becos_a_becos_a_corres_prediction/" + mode  + str(i) + ".pt")

    df = pd.DataFrame(tracks)
    df.insert(0, "pair", pairs)
    df.to_csv("shape_matching_results_becos_a_becos_a_" + mode + ".csv", index=False)