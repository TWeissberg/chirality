...
data_path = "/lustre/scratch/data/tweissbe_hpc-becos-all/full_full/test"
model_path_iccv = "/lustre/scratch/data/tweissbe_hpc-becos-all/models/iccv/becos_h/files/model.pt"
model_path_siggraph = "/lustre/scratch/data/tweissbe_hpc-becos-all/models/siggraph/becos_h/files/model.pt"

animal_list = ["80", "69", "22", "140", "102", "70", "136", "125", "65", "101", "78", "61", "109", "14", "6", "59", "35", "124", "134", "48", "64", "2", "26",\
 "95", "58", "77", "103", "32", "21", "52", "111", "88", "31", "28", "126", "116", "122", "24", "99", "90", "117", "50", "141", "115", "75", "60", "34", "81", "5", "113", "97", "130", "3",\
 "96", "17", "131", "1", "128", "91", "9", "108", "23", "118", "57", "87", "94", "25", "45", "38", "20", "121", "66", "29", "37", "89", "19", "0", "40", "112", "56", "104", "62", "44", "8",\
 "123", "74", "105", "42", "98", "82", "133", "41", "43", "106", "76", "12", "86", "110", "68", "137", "79", "7", "72", "15", "53", "4", "18", "33", "139", "129"]
human_list = ["67", "39", "11", "138", "120", "85", "114", "83", "63", "16", "36", "127", "13", "49", "135", "30", "132", "84", "46", "51", "47", "100", "73", "107", "55", "54", "93", "92", "119", "71", "27", "10"]
...
with torch.no_grad():
    for i in tqdm(os.listdir(data_path)):
        if i in animal_list:
            ...
            geodesic_0 = compute_geodesic_distmat(mesh0.vert, mesh0.face)
            geodesic_0 = torch.tensor(geodesic_0).to(device=device)
            ...
            if mode == "diff3f":
                ...
                feature0 = torch.load(full_path + "/0_features/diff3f_features.pt").to(device=device)[None,...]
                feature1 = torch.load(full_path + "/1_features/diff3f_features.pt").to(device=device)[None,...]
                ...
                # mask_cos_similarity0 = torch.mm(mask_feature0, mask_feature0.t())
                # mask_cos_similarity1 = torch.mm(mask_feature1, mask_feature1.t())
                ...
                chirality_feature0, feature0, _ = model(combined_feature0.unsqueeze(0).float())
                chirality_feature1, feature1, _ = model(combined_feature1.unsqueeze(0).float())
            ...
            mask0 = cosine_similarity(chirality_feature0[0], -chirality_feature0[0]).to(device=device)
            cos_similarity0 = cosine_similarity(feature0[0], feature0[0]).to(device=device)
            cos_similarity0[mask0<0] = -1

            mask1 = cosine_similarity(chirality_feature1[0], -chirality_feature1[0]).to(device=device)
            cos_similarity1 = cosine_similarity(feature1[0], feature1[0]).to(device=device)
            cos_similarity1[mask1<0] = -1


            #cos_similarity0 = cosine_similarity(feature0[0], feature0[0]).to(device=device) * cosine_similarity(chirality_feature0[0], -chirality_feature0[0]).to(device=device)
            #cos_similarity1 = cosine_similarity(feature1[0], feature1[0]).to(device=device) * cosine_similarity(chirality_feature1[0], -chirality_feature1[0]).to(device=device)
            #cos_similarity = torch.randn(mesh0.vert.shape[0], mesh1.vert.shape[0]).to(device=device)
            print("similarity computation finished!")

            gt_corres0 = torch.tensor(np.load(full_path + "/0_orig_to_flip.npy")).to(device=device)
            gt_corres1 = torch.tensor(np.load(full_path + "/1_orig_to_flip.npy")).to(device=device)

            if len(gt_corres0.shape) > 1:
                faces = torch.from_numpy(mesh0.face).to(device = device)
                gt_corres0 = faces[gt_corres0[:, 0].long(), torch.argmax(gt_corres0[:, 1:], -1)]

            if len(gt_corres1.shape) > 1:
                faces = torch.from_numpy(mesh1.face).to(device = device)
                gt_corres1 = faces[gt_corres1[:, 0].long(), torch.argmax(gt_corres1[:, 1:], -1)]

            verts0 = torch.tensor(mesh0.vert).to(device=device)
            verts1 = torch.tensor(mesh1.vert).to(device=device)

            result_dict0 = compute_acc(verts0, verts0, geodesic_0, cos_similarity0, gt_corres0)
            result_dict1 = compute_acc(verts1, verts1, geodesic_1, cos_similarity1, gt_corres1)

            print("final results computed finished!")

            logs_step0 = {k: to_numpy(v) for k, v in result_dict0.items()}
            logs_step1 = {k: to_numpy(v) for k, v in result_dict1.items()}

            tracks0.append(logs_step0)
            tracks1.append(logs_step1)
            pairs0.append(i)
            pairs1.append(i)

            torch.save(cos_similarity0.argmax(-1), "/lustre/scratch/data/s95wwang_hpc-chirality_image/self_matching_becosha_corres_prediction/" + mode  + str(i) + "_0.pt")
            torch.save(cos_similarity1.argmax(-1), "/lustre/scratch/data/s95wwang_hpc-chirality_image/self_matching_becosha_corres_prediction/" + mode  + str(i) + "_1.pt")

    df = pd.DataFrame(tracks0)
    df.insert(0, "pair", pairs0)
    df.to_csv("self_matching_0_results_becos_h_becos_a_" + mode + ".csv", index=False)

    df = pd.DataFrame(tracks1)
    df.insert(0, "pair", pairs1)
    df.to_csv("self_matching_1_results_becos_h_becos_a_" + mode + ".csv", index=False)