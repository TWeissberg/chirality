...
with torch.no_grad():
    for i in tqdm(os.listdir(data_path)):
        if i in human_list:
            ...
            torch.save(cos_similarity0.argmax(-1), "/lustre/scratch/data/s95wwang_hpc-chirality_image/self_matching_becoshh_corres_prediction/" + mode  + str(i) + "_0.pt")
            torch.save(cos_similarity1.argmax(-1), "/lustre/scratch/data/s95wwang_hpc-chirality_image/self_matching_becoshh_corres_prediction/" + mode  + str(i) + "_1.pt")

    df = pd.DataFrame(tracks0)
    df.insert(0, "pair", pairs0)
    df.to_csv("self_matching_0_results_becos_h_becos_h_" + mode + ".csv", index=False)

    df = pd.DataFrame(tracks1)
    df.insert(0, "pair", pairs1)
    df.to_csv("self_matching_1_results_becos_h_becos_h_" + mode + ".csv", index=False)