...
model_path_iccv = "/lustre/scratch/data/tweissbe_hpc-becos-all/models/iccv/becos/files/model.pt"
model_path_siggraph = "/lustre/scratch/data/tweissbe_hpc-becos-all/models/siggraph/becos/files/model.pt"
...
with torch.no_grad():
    for i in tqdm(os.listdir(data_path)):
        ...
        torch.save(cos_similarity0.argmax(-1), "/lustre/scratch/data/s95wwang_hpc-chirality_image/self_matching_becos_corres_prediction/" + mode + str(i) + "_0.pt")
        torch.save(cos_similarity1.argmax(-1), "/lustre/scratch/data/s95wwang_hpc-chirality_image/self_matching_becos_corres_prediction/" + mode  + str(i) + "_1.pt")

    df = pd.DataFrame(tracks0)
    df.insert(0, "pair", pairs0)
    df.to_csv("self_matching_0_results_becos_" + mode + ".csv", index=False)

    df = pd.DataFrame(tracks1)
    df.insert(0, "pair", pairs1)
    df.to_csv("self_matching_1_results_becos_" + mode + ".csv", index=False)