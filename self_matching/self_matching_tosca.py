...
def compute_acc(verts1, verts2, geo_matrix, cos_similarity, gt_corres):
    ...
    for each_ratio in ratio_list:
        ...
        val = make_soft_label(label, verts2, ratio=each_ratio).to(device)
        val_geo = make_soft_label_geo(label, verts2, geo_matrix, ratio=each_ratio).to(device)
...
def make_soft_label(label_origin, xyz2, ratio=0.5):
    if ratio == 0.0:
        ...
        label_origin = label_origin.to("cpu")
        xyz2 = xyz2.to("cpu")
...
def label_ACC_percentage_for_inference(label_in, label_gt):
    ...
    label_in = label_in.cpu()

    element_product = torch.mul(label_in, label_gt.cpu())
    N1 = label_in.shape[0]
...
data_path = "/lustre/scratch/data/tweissbe_hpc-becos-all/tosca_o/test_old"
model_path_iccv = "/lustre/scratch/data/tweissbe_hpc-becos-all/models/iccv/smal_o/files/model.pt"
model_path_siggraph = "/lustre/scratch/data/tweissbe_hpc-becos-all/models/siggraph/smal_o/files/model.pt"
...
with torch.no_grad():
    for i in tqdm(os.listdir(data_path)):
        ...
        torch.save(cos_similarity0.argmax(-1), "/lustre/scratch/data/s95wwang_hpc-chirality_image/self_matching_tosca_corres_prediction/" + mode  + str(i) + "_0.pt")
        torch.save(cos_similarity1.argmax(-1), "/lustre/scratch/data/s95wwang_hpc-chirality_image/self_matching_tosca_corres_prediction/" + mode  + str(i) + "_1.pt")

    df = pd.DataFrame(tracks0)
    df.insert(0, "pair", pairs0)
    df.to_csv("self_matching_0_results_tosca_" + mode + ".csv", index=False)

    df = pd.DataFrame(tracks1)
    df.insert(0, "pair", pairs1)
    df.to_csv("self_matching_1_results_tosca_" + mode + ".csv", index=False)